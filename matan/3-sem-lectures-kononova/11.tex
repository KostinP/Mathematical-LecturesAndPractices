...
\begin{document}
    \begin{Reminder}[т. Гурса]
        \[f \in H(\Omega \setminus \{p\}),\q f \in C(\Omega)\]
    \end{Reminder}
    ...
    \begin{Consequence} [из т. Гурса (т. Коши для ломаной)]
        ...
        \[f \text{ --- диф-ма } \forall z \in \Omega \setminus \{p\},\q f \in C(\Omega)\]
        \begin{figure}[H]
          \centering
          \includegraphics[width=5cm]{11_1}
        \end{figure}
        ...
        \begin{figure}[H]
          \centering
          \includegraphics[width=5cm]{11_2}
        \end{figure}
    \end{Consequence}
    ...
    \begin{Theorem}[Коши]
        \[L \text{ --- спрям. замк. кривая, простая},\q L = \d D, \q D \subset \Omega\]
        \begin{figure}[H]
          \centering
    ...
    \begin{Example}
        ...
        \begin{figure}[H]
          \centering
          \includegraphics[width=3cm]{11_6}
        \end{figure}
    \end{Example}
    ...
    \begin{proof}
        ...
        \begin{figure}[H]
          \centering
          \includegraphics[width=8cm]{11_8}
        \end{figure}
        \[\{z_j\}^N_{j = 1} \text{ --- точки на L: } L = \bigcup_{j = 1}^N l_j \]
        \[\us{\text{длина кусочка кривой}}{\abs{l_j}} < \delta\]
        ...
        \abs{\sum_{j = 1}^N \int_{l_j} f(z)dz - \sum_{j = 1}^N  \int_{p_j} f(z)dz} \leq}\]
        \begin{figure}[H]
          \centering
          \includegraphics[width=3.5cm]{11_10}
        \end{figure}
        ...
        \end{figure}
        \[\leq \abs{\int_{l_j} (f(z) - f(z_j))dz} +
        \abs{\int_{P_j}(f(z) - f(z_j))dz } \leq\]
        \[\leq \max_{z \in l_j} \abs{\ub{< \E}{f(z) - f(z_j)}}\cdot
        \abs{l_j} + \max_{z \in P_j} \abs{\ub{< \E}{f(z) - f(z_j)}} \cdot \abs{P_j} \]
        \[\max < \mathcal{E} (\text{т.к. }  \abs{z - z_j} < \delta)\]
        \[(*)\ \abs{\int_L f(z)dz} \leq \sum_{j = 1}^N \mathcal{E} \abs{l_j} +
        \mathcal{E} \abs{P_j} = \mathcal{E} (\abs{L} + \ub{\leq |L|}{\abs{P}}) \leq 2 \mathcal{E} \abs{L}\]
    ...
    \begin{Theorem}[интегральная формула Коши]
        \[L = \d D,\q D \subset \Omega,\q f \text{ --- диф-ма } \forall z \in \Omega\]
        ...
        \end{figure}
        \[\text{Тогда}\q \frac{1}{2\pi i}\int_L \frac{f(\xi)}{\xi - z_0}d\xi = \begin{cases}
            f(z_0), & z_0 \in D\\
    ...
    \begin{Proof}
        ...
        \[\widetilde{L} = L_+ \cup \gamma_+ \cup \gamma_- \cup \delta_r^-\]
    ...
    \begin{Theorem}[Коши о сост. контуре]
        \[L_1, ..., L_p \text{ --- простые замк. кривые, не пересекаются:}\]
        \begin{figure}[H]
          \centering
    ...
    \begin{Proof}[P = 2]\
        \begin{figure}[H]
          \centering
          \includegraphics[width=6cm]{11_17}
        \end{figure}
    ...
    \begin{Proof}
        ...
        \frac{f(\xi)}{\xi - z_0} d\xi =\]
        \[= \frac{1}{2\pi i }\int_0^{2\pi} \frac{f(z_0 +
        r e^{it} )}{r e^{it} } r i e^{it} dt = \frac{1}{2\pi}\int_0^{2\pi}  f(z_0 + re^{it} )dt\]
    \end{Proof}
    ...
    \begin{proof}
       ...
       \[= \abs{\frac{1}{\Delta z} \left[ \int_{z_0}^{z + \Delta z} f(\xi)d\xi -
        ...
        \[\text{т.е. } F \text{ --- диф-ма } \forall  z \in \Omega\]
        \[F'(z) = f(z) \in  C(\Omega)\]
        \[F \in H(\Omega)\]
    \end{proof}
    ...
    \begin{Theorem}[было]
        \[\text{Если } f(z) = \sum_{k = 0}^\infty a_k (z - z_0)^k, \q R \text{ --- рад. сх},\q z \in D(a, R)\]
    ...
    \begin{Consequence} [н-ва Коши]
        ...
        {\xi^{k + 1} } } \leq \frac{1}{2\pi} \cdot \frac{M(\rho) \cdot 2 \pi \rho}{\rho^{k + 1}} = \frac{M(\rho)}{\rho^k}\]
