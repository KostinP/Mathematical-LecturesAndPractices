...
\begin{properties}
    \begin{enumerate}
        ...
        \item 4) $\forall \uptau_1, \uptau_2:\ S_*(\uptau_1) \leqslant S^*(\uptau_2)$
    \end{enumerate}
\end{properties}
...
\begin{Theorem} [критерий Дарбу, другая форма]
    \[f \in R [a,b] \lra \forall \E > 0,\ \e \updelta > 0: \forall \uptau: \uplambda(\uptau) < \updelta \ \sum\limits_{k=0}^{n-1} \upomega(f, d_k) \Delta_k < \E\]
...
\begin{proof}
    ...
    \[\Rightarrow f*g=\frac{1}{4}((f+g)^2-(f-g)^2) \in R[a,b]\]
\end{proof}
...
\begin{proof}
    $\upmu=\frac{\int\limits^a_b f}{b-a} \in [m,M]$ (по предыдущему неравенству)
\end{proof}
...
\begin{definition}
    $E \subset \R$, E - невырожденный промежуток, $f: E \rightarrow \R,\ \forall \upalpha, \upbeta \in E: \upalpha < \upbeta,$ $f \in R[\upalpha, \upbeta]$, для $a \in E$ (фиксированного) $F(x):=\int\limits_a^x f(t) dt$ ($F: E \rightarrow \R$)
\end{definition}
...
\begin{proof}
    ...
    |F(x+h)-F(x)| \leqslant |\int\limits_x^{x+h} f| \leqslant \int\limits_x^{x+h} |f| \leqslant M |h|$
    
    Кроме того, $\forall \E > 0,\ \updelta = \frac{\E}{M}$ если $|h| < \updelta \Rightarrow |F(x+h)-F(x)|  < \E$
    \\
    2) Рассмотрим $|\frac{F(x_0+h)-F(x_0)}{h}-f(x_0)| = |\frac{1}{h} \int\limits_{x_0}^{x_0+h} f(t) dt - f(x_0) \frac{1}{h} \int\limits_{x_0}^{x_0+h} dt| =$ 
...
\begin{example}
    ...
    \begin{example}
        ...
        Попробуем использовать Н-Л. $F(t) |_0^2 = F(2) - F(0) = 2$. Неверно, потому что это не первообразная и даже не "почти первообразная". Поправим F(x):
...
\begin{example}
    ...
    I_m = 
     \begin{cases}
       \frac{\pi}{2} \frac{(m-1)!!}{m!!}, &\text{$m$ - четное}\\
       \frac{(m-1)!!}{m!!}, &\text{$m$ - нечетное}
     \end{cases}
    \]
\end{example}
...
\begin{Theorem}[Формула Валлиса]
    \[\lim\limits_{n \rightarrow \infty} \frac{2*2*4*4*...*(2n)(2n)}{1*3*3*5*5...(2n-1)(2n+1)} = \frac{\pi}{2}$ (или $\lim\limits_{n \rightarrow \infty} \frac{1}{n} (\frac{(2n)!!}{(2n-1)!!})^2 = \pi)\]
\end{Theorem}
...
\begin{examples}
    ...
    \includegraphics[scale=0.4]{Kantor}
