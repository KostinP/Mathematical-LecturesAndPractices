...
\begin{Task}
    \[f: U \subset \R^2 \to \R \qq C^2\]
    \[S = \{(x, y, z) \in \R^3 \ \mid \ z = f(x, y) \}\]
    Найти кривизну Гаусса
    \[\text{через } \frac{\partial ^2 f}{\partial x^2}, \ 
    ...
    \end{pmatrix}\]
    Очевидно, что они ЛНЗ.
    \[\frac{\partial \varphi}{\partial x} \q \frac{\partial \varphi}{\partial y}\]
    
    \[I = \begin{pmatrix}
        1 + f_x^2 & f_xf_x\\
        f_xf_y & 1 + f_y^2
    \end{pmatrix}\]
    \[\RNumb{2} = \frac{1}{\sqrt{1 + f_x^2 + f_y^2}}\begin{pmatrix}
        f_{xx} & f_{xy}\\
        f_{xy} & f_{yy}  
    \end{pmatrix}\]
...
\begin{Reminder}
    \[\gamma : \ (0, 1) \to \R^3 \qq C^2\]
    \[t \to (\gamma_1(t), \gamma_2(t), \gamma_3(t)) \]
...
\begin{Task}
    ...
    %Очев. параметризация $(x, f(x))$\\
    Мы знаем, что для любой кривой есть нат. парам.
    \[\gamma(t) = (t, f(t)) \qq \gamma : [0, 1] \to \R^3\]
    \[\exists \varphi : [0, l(\gamma)] \to [0, 1]\]
    \[s \to \varphi(s) = l\]
    \[\widetilde{\gamma} = \gamma \circ \varphi(s)\]
    \[\abs{\dot{\widetilde{\gamma}}(s)} = 1 \q \forall  s \in [0, l(\gamma)]\]
    \[\widetilde{\gamma}(s) = (\varphi(s), f(\varphi(s)))\]
    \[\dot{\widetilde{\gamma}} = (\dot{\varphi}(s),\ \dot{\varphi}(s)f'(\varphi(s)))\]
    Мы знаем, что $\dot{\varphi}^2 + \dot{\varphi}^2 f'^2_{(\varphi(s))} = 1 $
