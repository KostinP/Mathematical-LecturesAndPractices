...
\begin{document}
	...
	\begin{Consequence} [теорема об открытом отображении]
	...
	\begin{Theorem}[о неявном отображении]
		...
		\[\Phi : W \to \R^m \q \Phi \in C^{1} (W) \q \Phi(a, b) = 0_m \in \R^m.\q \det(\Phi'_y(a, b)) \neq 0\]
		Тогда:
		\[\exists U_a \text{ --- окр. } a \q V_b \text{ --- окр. }b\]
		\begin{enumerate}
			\item $\forall x \in U \q \exists ! y \in V : \Phi (x, y) = 0$
				\[(\text{Обозначим } \varphi(x) = y \text{ (который ед.)})\]
				\[\text{т.е } \exists \text{ отобр. } \varphi : U \to V :  \q \Phi(x, \varphi(x)) = 0\]
			\item $\varphi \in C^{1}(U \to V)$
			\item $\varphi'(x) = - \left(\Phi_y'(x, y)\right)^{-1} \left(\Phi'_x(x, y)\right)
				\bigg|_{y = \varphi(x)} =$
